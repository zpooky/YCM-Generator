...
def main():
    ...
    with tempfile.NamedTemporaryFile(mode="rw") as c_build_log:
        with tempfile.NamedTemporaryFile(mode="rw") as cxx_build_log:
            ...
            generate_conf(["-x", lang] + flags, config_file, project_dir)
...
def generate_cc_conf(flags, config_file, project_dir):
...
def generate_ycm_conf(flags, config_file, project_dir):
    ...
    with open(template_file, "r") as template:
        with open(config_file, "w") as output:
            ...
            for line in template:
                if (line == "compilation_database_folder = ''\n"):
                    output.write("compilation_database_folder = '{}'\n".format(project_dir))
                elif(line == "    # INSERT FLAGS HERE\n"):
